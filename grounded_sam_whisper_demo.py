...
def load_image(image_path):
    # load image
    image_path = image_path["image"]  # image_path = image
...
# Only used by gui
def ground_sam_whisper(input_image, input_audio, box_threshold, iou_threshold, text_threshold):
    ...
    device = "cuda"
    speech_file = input_audio

    # load speech
    whisper_model = whisper.load_model("base")
    speech_text, speech_language = speech_recognition(speech_file, whisper_model)
    ...
    fig = plt.gcf()
    fig.canvas.draw()
    image_out = np.array(fig.canvas.renderer.buffer_rgba())
    image_out = image_out.resize(size)
    rgb_image = cv2.cvtColor(np.array(image_out), cv2.COLOR_BGR2RGB)
